...
$HTMLFormat = $HTMLFormat + "#myInput {background-image: url('$SearchIconPath'); background-position: 0px 0px; background-repeat: no-repeat; width: 70%; font-size: 16px; padding: 12px 20px 12px 40px; border: 1px solid #ddd; margin-bottom: 12px;}"
...
$VMBackupDetailHTML = $VMBackupDetail | Sort-Object VM_VCenter, VM_Name | ConvertTo-Html -Head $HTMLFormat -Title "VM Backup Job Report for $VCentersString as of $ExecutionStamp" -Body "<center><img src=`"$path\css\WegmansLogo.png`" alt=`"Wegmans Logo`"><p><H2>Backup Job Report for $VCentersString as of $ExecutionStamp</H2><p><input type=`"text`" id=`"myInput`" onkeyup=`"myFunction()`" placeholder=`"Search for VM names..`" title=`"Type in a VM name`"></center> <script>function myFunction() {  var input, filter, table, tr, td, i;  input = document.getElementById(`"myInput`");  filter = input.value.toUpperCase();  table = document.getElementById(`"myTable`");  tr = table.getElementsByTagName(`"tr`");  for (i = 0; i < tr.length; i++) {    td = tr[i].getElementsByTagName(`"td`")[1];    if (td) {      if (td.innerHTML.toUpperCase().indexOf(filter) > -1) {        tr[i].style.display = `"`";      } else {        tr[i].style.display = `"none`";      }    }        }}</script>"
...
$VMSnapMirrorBackupDetailHTML = $VMSnapMirrorBackupDetail | Sort-Object VM_VCenter, VM_Name | ConvertTo-Html -Head $HTMLFormat -Title "VM SnapMirror Backup Job Report for $VCentersString as of $ExecutionStamp" -Body "<center><img src=`"$path\css\WegmansLogo.png`" alt=`"Wegmans Logo`"><p><H2>VM SnapMirror Backup Job Report for $VCentersString as of $ExecutionStamp</H2><p><input type=`"text`" id=`"myInput`" onkeyup=`"myFunction()`" placeholder=`"Search for VM names..`" title=`"Type in a VM name`"></center> <script>function myFunction() {  var input, filter, table, tr, td, i;  input = document.getElementById(`"myInput`");  filter = input.value.toUpperCase();  table = document.getElementById(`"myTable`");  tr = table.getElementsByTagName(`"tr`");  for (i = 0; i < tr.length; i++) {    td = tr[i].getElementsByTagName(`"td`")[1];    if (td) {      if (td.innerHTML.toUpperCase().indexOf(filter) > -1) {        tr[i].style.display = `"`";      } else {        tr[i].style.display = `"none`";      }    }        }}</script>"
...
$BackupJobDetail | Sort-Object JobName | ConvertTo-Html -Head $HTMLFormat -Title "Backup Job Report as of $ExecutionStamp" -Body "<center><img src=`"$path\css\WegmansLogo.png`" alt=`"Wegmans Logo`"><p><H2>Backup Job Report for $VCentersString as of $ExecutionStamp</H2>" | Out-File -FilePath $OutputPath
#
$OutputPath = $path + "rpt_vmswithnobackups" + $FileExt
$VMsWithNoBackups | Sort-Object VM_VCenter, VM_Name | ConvertTo-Html -Head $HTMLFormat -Title "VMs With No Backup Jobs as of $ExecutionStamp" -Body "<center><img src=`"$path\css\WegmansLogo.png`" alt=`"Wegmans Logo`"><p><H2>VMs With No Backup Jobs as of $ExecutionStamp</H2>" | Out-File -FilePath $OutputPath
#
$OutputPath = $path + "rpt_vmwaredatastorebackujobs" + $FileExt
$VMWareDatastoreBackupJobsHTML = $VMWareDataStoreBackupJobsDetail | Sort-Object VM_VCenter, VM_Name | ConvertTo-Html -Head $HTMLFormat -Title "Datastore Backup Job Report for $VCentersString as of $ExecutionStamp" -Body "<center><img src=`"$path\css\WegmansLogo.png`" alt=`"Wegmans Logo`"><p><H2>Datastore Backup Job Report for $VCentersString as of $ExecutionStamp</H2><p><input type=`"text`" id=`"myInput`" onkeyup=`"myFunction()`" placeholder=`"Search for Datastore names..`" title=`"Type in a Datastore name`"></center> <script>function myFunction() {  var input, filter, table, tr, td, i;  input = document.getElementById(`"myInput`");  filter = input.value.toUpperCase();  table = document.getElementById(`"myTable`");  tr = table.getElementsByTagName(`"tr`");  for (i = 0; i < tr.length; i++) {    td = tr[i].getElementsByTagName(`"td`")[0];    if (td) {      if (td.innerHTML.toUpperCase().indexOf(filter) > -1) {        tr[i].style.display = `"`";      } else {        tr[i].style.display = `"none`";      }    }        }}</script>"
...
$TOCHTML += "<head>"
$TOCHTML += $HTMLFormat
$TOCHTML += "</head><body><center><img src=`"$path\css\WegmansLogo.png`" alt=`"Wegmans Logo`"><p>"
